...
public class SmallBamWriterTest extends BaseTest {
    ...
    protected void testReadAndWrite(final String inputPath, final String outputPath, boolean enableGcs, boolean enableCloudExec) throws IOException {
        ...
        PCollection<MutableGATKRead> input = ingestReadsAndGrabHeader(pipeline, inputPath);
    ...
    private PCollection<MutableGATKRead> ingestReadsAndGrabHeader(final Pipeline pipeline, String filename) throws IOException {
        ...
        if (BucketUtils.isCloudStorageUrl(filename)) {
            ...
            try( ReadsDataSource readsSource = new ReadsDataSource(new File(filename)) ) {
                ...
                for ( MutableGATKRead read : readsSource ) {
                    reads.add(read);
                }
                return pipeline.apply(Create.of(reads).withName("input ingest"));
            }
        }
    }
